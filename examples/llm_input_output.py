...
def __(gpt2_tokenizer, mo, prompt_tokens):
    ...
    mo.md(f"""From tokens we can know the subwords of the prompt (generated by BPE): **{subwords}**\n
...
def __(mo, prompt_tokens_tensor, text_generator, torch):
    ...
    mo.md(f"Feed `prompt_tokens_tensor` into the GPT2 Neural Network, we get the **logits**, a tensor with shape **{logits.shape}**")
    return (logits,)
...
def __(logits, mo):
    ...
    mo.md(f"""Select the last logits only (with shape **{logits_last.shape}**), which are the next token's logits (can be treated as unnormalized probability) on the vocabulary set (contains 50,527 items)""")
    return (logits_last,)
...
def __(logits_last, mo, text_generator, torch):
    ...
    mo.md(f"""Apply topK sampling and temperature scaling to the last logit, we get the next token id:
    **{next_token_id}**""")
...
@app.cell
def __(mo):
    mo.md("""Powered by [ToyLLM](https://github.com/ai-glimpse/toyllm) | Author: [MathewShen](https://github.com/shenxiangzhuang).""").center()
    return
